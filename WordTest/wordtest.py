...
while 1:
    ...
    def extract_last_word(text):
        """
        문자열에서 두 번째 단어 이후를 추출합니다.

        Args:
            text (str): 입력 문자열.

        Returns:
            str: 두 번째 단어이후 모든 내용. 두 번째 단어가 없으면 빈 문자열 반환.
        """
        # 문자열을 공백 기준으로 분리
        words = text.split()
    ...
    if __name__ == "__main__":
        ...
        # 답안지 양식 불러오기 (동일일)
        doc_sol = Document('C:/pythonprojects/WordTest/Word Test Format.docx')
        ...
        for txt in result:
            ...
            sollist.append(extract_last_word(txt))
        ...
        table = doc.tables[0]
        table_sol = doc_sol.tables[0]
2    for word in wordlist[half:]:
            table.cell(cRow, 4).text = word    
            cRow += 1

        cRow = 1
        for sol in sollist[half:]:
            table_sol.cell(cRow, 4).text = sol
            cRow += 1
        ...
        doc_sol.save(f'{filename}_sol.docx')
